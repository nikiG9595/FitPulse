...
public class AuthController {
    ...
    @GetMapping("/register") public String register(Model model) { model.addAttribute("registerRequest", new RegisterRequest()); return "auth/register"; }
    ...
    public String doRegister(@Valid @ModelAttribute RegisterRequest registerRequest, BindingResult bindingResult, Model model) {
        ...
        try { userService.register(registerRequest); return "redirect:/login?registered"; }
        catch (FitPulseException e) { model.addAttribute("globalError", e.getMessage()); return "auth/register"; }
    }
}
